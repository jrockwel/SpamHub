...
class parser:
    ...
    def all_tfidf(self,totalfiles,totalwords):
        ...
        for instance in totalfiles:
            count += 1
            newinst, idfdic = self.tfidf_vector(totalfiles,instance,totalwords,idfdic)
    ...
    def tfidf_vector(self, totalfiles, inst, allwords,idfdic):

        vector = []
        tfdic = {}
        for word in allwords:
            score, idfdic, tfdic = self.tfidf_score(word, inst[1], totalfiles,idfdic,tfdic)
            vector.append(score)

        newinst = instance(inst[0], vector,inst[2],True)

        return newinst, idfdic

    def tfidf_score(self, term, words, totalfiles,idfdic,tfdic):

        try:
            tf = tfdic[term]
        except KeyError:
            tf, tfdic = self.tf_calc(term, words,tfdic)
        try:
            idf = idfdic[term]
        except KeyError:
            idf, idfdic = self.idf_calc(term, totalfiles,idfdic)

        return tf * idf, idfdic, tfdic

    def tf_calc(self, term, words,tfdic):
    ...
    def idf_calc(self, term, totalfiles,idfdic):
