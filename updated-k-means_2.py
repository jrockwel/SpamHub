...
def clust_overview(clusters,folderpath,direc, seed, k, distfunc,vectype, ari,subjects, minvar, lendataset):
    ...
    file.write("Dataset:  " + str(direc) +" ("+ str(lendataset) + ')' +  '\n')
    ...
    if minvar != None:
        file.write('intra-cluster variance: ' + str(minvar) + '\n')
...
def contin_table (clusters,instances,subjects):
    ...
    #making contin_table
    table = [[0 for _ in range(len(subjects))]for _ in range(len(subjects))]
...
def main():
    ...
    if k == 'same':
        ...
        clust_overview(clusters, path, direc, seed, k, distfunc, vectype, ari, subjects, None, len(instances))
    ...
    if k == 'minvar':
        ...
        cluster_seed_pair = []
        rands = random.sample(range(1, 99), 5)
        count = 0
        print('percent done: 0')
        for j in range (5):
            cluster_k_pair = []
            for i in range (1,((len(instances)))//30):

                print(i)
                centroids = randompick(instances, rands[j], i)
            ...
            print(rands[j])
            for i in range(len(cluster_k_pair)):
                print('{' + str(len(cluster_k_pair[i][0].clusters)) + ', ' + str(cluster_k_pair[i][1]), "}, ", end='')

            cluster_k_pair = cluster_k_pair[1:]

            right_clust = max(cluster_k_pair, key= lambda x: x[2])

            cluster_seed_pair.append(right_clust)

            count += 1
            print(count/5)
        print('\n\n\noverseeds:')
        for pair in cluster_seed_pair:
            print(pair)
        clusters = min(cluster_seed_pair, key = lambda x: x[1])[0]
        minvar = clusters.intrasum
        ...
        clust_overview(clusters, path, direc, seed, k, distfunc, vectype, ari, subjects,minvar,len(instances))
        writeclusterdoc(clusters, path, direc)


    if k != 'same' and k !='minvar':
        ...
        clust_overview(clusters, path, direc, seed, k, distfunc,vectype, ari,subjects, variance, len(instances))
        writeclusterdoc(clusters, path, direc)
